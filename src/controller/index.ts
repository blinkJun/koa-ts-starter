...
export default class Index{
    ...
    async getPage(ctx:Context,next:Next):Promise<void>{
        await next()
        ctx.status = 200
        ctx.body = '<script src="https://cdn.bootcdn.net/ajax/libs/axios/0.21.1/axios.js"></script><script>axios.post("http://localhost:3000/index/detail",{id:3,number:"2"})</script>'
    }
}